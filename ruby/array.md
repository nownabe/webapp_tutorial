...
### 配列の長さ
配列の長さ、つまり要素の数は`length`メソッドか`size`メソッドで取得できます。
...
### (3) 各要素を2倍するメソッド
配列の各要素を2倍した配列を返す`double_elements`メソッドを作ってください。
...
ファイルは`File.read(ファイル名)`で読み込むことができます。
...
英数字かどうかは、`ascii_only?`というメソッドで調べられます。
...
### (7) 文字をつなぐ
配列を使ってユーザの入力した文字列のそれぞれの文字を`=`(イコール)でつなぐプログラムを作成してください。
